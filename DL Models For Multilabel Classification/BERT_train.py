...
import time
from BERT_Preprocessor import get_time_dif
...
def train(args, model, train_iter, dev_iter, test_iter):
    ...
    optimizer = BertAdam(optimizer_grouped_parameters,
                         lr=args.bert_lr,
                         warmup=0.05,
                         t_total=len(train_iter) * args.bert_epochs)
    ...
    for epoch in range(args.bert_epochs):
        print('Epoch [{}/{}]'.format(epoch + 1, args.bert_epochs))
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if total_batch % 100 == 0:
                ...
                model.train()
            total_batch += 1
            if total_batch - last_improve > args.early_stop:
...
def test(config, model, test_iter):
    ...
    model.eval()
    start_time = time.time()
    test_acc, test_loss, test_f1 = evaluate(args, model, test_iter, test=True)
    msg = 'Test Loss: {0:>5.2},  Test Acc: {1:>6.2%}   Test F1 : {2:4f}'
